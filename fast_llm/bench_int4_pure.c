...
typedef struct int4_expanded_t int4_expanded_t;
extern int4_expanded_t* create_int4_expanded(const float* weights, int rows, int cols, int block_size);
extern void free_int4_expanded(int4_expanded_t* mat);
extern void matmul_int4_expanded_avx2(const float* A, const int4_expanded_t* B, float* C,
                                       int M, int N, int K);
...
int main() {
    ...
    printf("\n========================================\n");
    printf("  EXPANDED INT4 (Pre-dequantized) TEST\n");
    printf("========================================\n\n");
    ...
    /* Create expanded INT4 */
    printf("Expanding INT4 to INT8 (one-time cost)...\n");
    double expand_start = get_time_ms();
    int4_expanded_t* W_up_int4 = create_int4_expanded(W_up_float, 2*intermediate, hidden, 64);
    int4_expanded_t* W_down_int4 = create_int4_expanded(W_down_float, hidden, intermediate, 64);
    double expand_time = get_time_ms() - expand_start;
    printf("Expansion time: %.1f ms (amortized over inference)\n", expand_time);
    ...
    for (int w = 0; w < 5; w++) {
        for (int layer = 0; layer < num_layers; layer++) {
            ...
            matmul_dequantized_asm_style(norm_out, &W_up_int8, output_up, 1, 2*intermediate, hidden);
            swiglu_avx2(output_up, output_up + intermediate, output_up, intermediate);
            matmul_dequantized_asm_style(output_up, &W_down_int8, output_down, 1, hidden, intermediate);
    ...
    /* Benchmark expanded INT4 */
    printf("\n2. Benchmarking expanded INT4...\n");
    for (int i = 0; i < hidden; i++) hidden_state[i] = 0.01f;
    
    for (int w = 0; w < 3; w++) {
        for (int layer = 0; layer < num_layers; layer++) {
            rms_norm_avx2(hidden_state, norm_out, hidden, 1e-5f);
            matmul_int4_expanded_avx2(norm_out, W_up_int4, output_up, 1, 2*intermediate, hidden);
            swiglu_avx2(output_up, output_up + intermediate, output_up, intermediate);
            matmul_int4_expanded_avx2(output_up, W_down_int4, output_down, 1, hidden, intermediate);
            for (int j = 0; j < hidden; j++) hidden_state[j] += output_down[j];
        }
    }
    ...
    printf("   INT4 expanded: %.1f tok/sec\n", int4_tok_sec);
    ...
    if (int4_tok_sec >= 50.0) {
        printf("🎉 50 TOK/SEC ACHIEVED WITH EXPANDED INT4!\n");
    } else if (int4_tok_sec >= int8_tok_sec * 0.9) {
        printf("⚠️ INT4 similar speed to INT8 but not 50+\n");
        printf("   Need %.1f more tok/sec\n", 50.0 - int4_tok_sec);
    } else {
        printf("❌ Expanded INT4 slower than INT8\n");
        printf("   The expansion overhead isn't worth it\n");
    }
    
    printf("\nNote: Expanded INT4 uses same memory as INT8\n");
    printf("      So no bandwidth advantage\n");
    
    /* Cleanup */
    aligned_free(W_up_float);
    aligned_free(W_down_float);
    free_int4_expanded(W_up_int4);
    free_int4_expanded(W_down_int4);
