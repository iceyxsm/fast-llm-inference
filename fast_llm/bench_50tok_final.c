...
void benchmark_kernel(const char* name,
    ...
    printf("%-20s: %6.3f ms | %5.1f GFLOPS\n", name, ms_per, gflops);
}
...
int main() {
    ...
    printf("\n========================================\n");
    printf("  50 TOK/SEC FINAL ATTEMPT\n");
    printf("========================================\n\n");
    ...
    /* Gate+Up: [2*inter, hidden] = [16384, 3072] - LARGE N */
    ...
    /* Down: [hidden, intermediate] = [3072, 8192] - SMALL N */
    W_down.rows = hidden;
    ...
    printf("Gate+Up (Large N = %d):\n", 2*intermediate);
    benchmark_kernel("6x16 ASM-Style", matmul_dequantized_asm_style,
                     input, &W_up, output_up, 1, 2*intermediate, hidden, iterations);
    benchmark_kernel("16x8 Large-N", matmul_dequantized_large_n,
                     input, &W_up, output_up, 1, 2*intermediate, hidden, iterations);
    ...
    /* Determine best combination and run full test */
    printf("\n========================================\n");
    printf("  FULL MODEL TEST\n");
    printf("========================================\n\n");
    
    /* Quick test to find best Gate+Up kernel */
    ...
    for (int tok = 0; tok < num_tokens; tok++) {
        for (int layer = 0; layer < 32; layer++) {
            /* Gate + Up - use best kernel */
            best_up(input, &W_up, output_up, 1, 2*intermediate, hidden);
    ...
    if (tok_per_sec >= 50.0) {
        ...
        printf("⚠️ Close! (80%+) - Need more work\n");
    } else {
        printf("❌ 50 tok/sec not reached\n");
        printf("Current best: 30 tok/sec with 6x16 kernel\n");
        printf("To reach 50 tok/sec need:\n");
        printf("  1. Assembly code (not intrinsics)\n");
        printf("  2. Quantized activations (INT8 x INT8)\n");
        printf("  3. Batched inference\n");
        printf("  4. Better thread utilization\n");
    }
    printf("\n");
