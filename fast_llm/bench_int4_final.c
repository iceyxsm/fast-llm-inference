/*
 * Final INT4 Benchmark
 * Optimized lookup-based INT4 vs INT8
 */
...
/* Optimized INT8 matmul with AVX2 */
void matmul_int8_avx2(const float* A, const int8_matrix_t* B, float* C,
                       int M, int N, int K) {
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            float sum = 0.0f;
            ...
            for (; k + 15 < K; k += 16) {
                ...
                for (int kk = 0; kk < 16; kk++) {
                    sum += A[i*K + k + kk] * B->weights[j*K + k + kk] * scale;
                }
            }
            /* Remainder */
            for (; k < K; k++) {
...
int main() {
    ...
    printf("\n========================================\n");
    printf("  FINAL INT4 vs INT8 BENCHMARK\n");
    printf("========================================\n\n");
    ...
    for (int t = 0; t < tokens; t++) {
        for (int layer = 0; layer < num_layers; layer++) {
            matmul_int8_avx2(hidden_state, &W_up_int8, output_up, 1, 2*intermediate, hidden);
            /* Simple SwiGLU */
            ...
            matmul_int8_avx2(output_up, &W_down_int8, output_down, 1, hidden, intermediate);
    ...
    for (int t = 0; t < tokens; t++) {
        for (int layer = 0; layer < num_layers; layer++) {
            matmul_int4_lookup(hidden_state, W_up_int4, output_up, 1, 2*intermediate, hidden);
            /* Simple SwiGLU */
            for (int i = 0; i < intermediate; i++) {
                float g = output_up[i];
                float u = output_up[i + intermediate];
            ...
            matmul_int4_lookup(output_up, W_down_int4, output_down, 1, hidden, intermediate);
            for (int j = 0; j < hidden; j++) hidden_state[j] += output_down[j];
        }
    }
    ...
    } else if (int4_tok_sec > int8_tok_sec * 1.1) {
        printf("⚠️ INT4 faster than INT8 but need %.1f more tok/sec\n", 50.0 - int4_tok_sec);
        printf("    Need further optimization\n");
    } else {
        printf("❌ INT4 unpacking overhead too high\n");
        printf("    INT4: %.1f tok/sec vs INT8: %.1f tok/sec\n", int4_tok_sec, int8_tok_sec);
        printf("\nAnalysis:\n");
        printf("- INT4 reduces memory bandwidth by 2x\n");
        printf("- But unpacking requires extra instructions\n");
        printf("- Net effect depends on compute vs memory bound\n");
        printf("- Currently compute-bound on unpacking\n");
    }
